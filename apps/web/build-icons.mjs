...
const outputDirPublic = path.join(cwd, 'public');
const outputDirComponent = path.join(cwd, 'components', 'icons');
const files = glob
  .sync('**/*.svg', {
    cwd: inputDir
  })
...
await writeIfChanged(path.join(outputDirPublic, 'sprite.svg'), spritesheetContent);
/**
...
await writeIfChanged(path.join(outputDirComponent, 'names.ts'), typesContent);
